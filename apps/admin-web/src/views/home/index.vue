  ...
  <div v-loading="loading">
    ...
    <div class="bottom-row">
      <div class="table-card wide">
        <div class="card-title">订单发货信息</div>
        <table>
          <thead>
            <tr>
              <th>时间</th>
          ...
          <tbody>
            <tr v-for="o in pendingOrders" :key="o.no">
              <td>{{ o.time }}</td>
              <td class="small">{{ o.no }}</td>
              <td>{{ o.buyer }}</td>
              <td>
                <span :class="['tag', o.statusCode === 'SHIPPED' ? 'green' : 'red']">{{ o.status }}</span>
              </td>
              ...
              <td>
                ...
                <span v-else class="done-text">已发货</span>
              </td>
            </tr>
            <tr v-if="pendingOrders.length === 0">
              <td colspan="8" class="empty">暂无发货订单</td>
            </tr>
          </tbody>
        </table>
      </div>
...
const { loading, shippingOrderId, stats, days, salesAmt, salesQty, refundAmt, refundQty, pendingOrders, lowStock, loadDashboard, shipFromDashboard } =
  useDashboard();
...
.done-text { color: #389e0d; font-weight: 600; }
