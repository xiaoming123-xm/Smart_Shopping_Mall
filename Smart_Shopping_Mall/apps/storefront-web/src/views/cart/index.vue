  ...
  <div class="cart">
    ...
    <div v-if="cart.items.length" class="list">
      <div v-for="it in cart.items" :key="it.product.id" class="item">
        <div class="cover">{{ it.product.cover }}</div>
      ...
      <div class="bar">
        ...
        <button class="checkout" @click="doCheckout">去结算（{{ cart.count }}件）</button>
      </div>
    </div>
...
function doCheckout() { checkout(router); }
</script>
<style scoped>
...
.cover { font-size: 36px; text-align: center; }
.price { color: #888; }
