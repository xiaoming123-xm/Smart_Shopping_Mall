  ...
  <div class="page">
    ...
    <el-card v-loading="loading">
      <el-table :data="list" border stripe>
        ...
        <el-table-column label="操作" width="200">
          <template #default="{ row }">
            <el-button link type="primary" size="small" @click="openValues(row)">值管理</el-button>
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑属性' : '新增属性'" width="460px">
      <el-form :model="form" label-width="90px">
        ...
        <el-form-item label="可搜索"><el-switch v-model="form.searchable" /></el-form-item>
...
import type { AttributeDTO, AttributeValue } from "@/api";
...
const form = reactive<AttributeDTO>({ name: "", type: "select", searchable: false, required: false, sort: 0 });
...
async function load() {
  loading.value = true;
  try { list.value = await loadAttributes(); }
  catch (e) { ElMessage.error(e instanceof ApiError ? e.message : "加载失败"); }
  finally { loading.value = false; }
}

function reset() { Object.assign(form, { id: undefined, name: "", type: "select", searchable: false, required: false, sort: 0 }); }
function openCreate() { reset(); dialogVisible.value = true; }
function openEdit(row: AttributeDTO) { Object.assign(form, { ...row }); dialogVisible.value = true; }
...
async function onDelete(row: AttributeDTO) {
  await ElMessageBox.confirm(`确认删除属性「${row.name}」？`, "提示", { type: "warning" }).catch(() => null);
...
async function onAddValue() {
  ...
  try {
    ...
    newValue.value = "";
    await load();
    current.value = list.value.find((a) => a.id === current.value?.id) || current.value;
